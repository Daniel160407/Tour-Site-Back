...
function Zd({adminMode: e}) {
    ...
    O.useEffect(() => {
        F.get("/tours/feedback", {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(i => {
            const l = i.data;
            n([]);
    ...
    const r = () => {
        ...
        if (i.value !== "" && l.value !== "") {
            ...
            i.value = "", l.value = "", F.post("/tours/feedback", s, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(u => {
                const a = u.data;
                n([]);
        ...
        F.delete(`/tours/feedback?comment=${i}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(l => {
            const s = l.data;
            n([]);
...
function Vg({tour: e, exit: t}) {
    const [n, r] = O.useState({...e}), [o, i] = O.useState(1e3), [l, s] = O.useState(1e3), u = c => {
        ...
        c.preventDefault(), F.put("/tours/adminpanel", n, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(() => t())
    };

    function d() {
        F.delete(`/tours/adminpanel?name=${n.name}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(() => t())
    }
...
function Kg() {
    ...
    function T(R) {
        R.preventDefault();
        const $ = new FormData;
        $.append("title", e), $.append("description", o), $.append("direction", n), $.append("duration", l), $.append("history", u), $.append("requirements", d), $.append("price", m), $.append("language", g), $.append("image", x), F.post("/tours/adminpanel", $, {headers: {"Content-Type": "multipart/form-data"}});
    ...
    O.useEffect(() => {
        if (!e) {
            F.get("https://ipinfo.io/json?token=d2261c6bcf22ce").then(N => {
                ...
                F.post("/statistics", C, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}})
            });
            const E = () => {
                const C = ((Date.now() - x.current) / 1e3 / 60).toFixed(2);
                F.put(`/statistics?time=${C}&clicks=${h.current}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}})
            };
        ...
        d && l && F.get(`/tours/tour?language=${d}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(E => {
            const N = E.data.tours.map(C => ({...C, imageData: C.imageData}));
            r(N), N.length !== 0 && w(Math.floor(Math.random() * N.length))
        })
    }, [d, l]), O.useEffect(() => {
        t !== void 0 && F.get(`/tours/tour/search?prefix=${t}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(E => {
            if (E !== void 0) {
                const N = E.data.tours.map(C => ({...C, imageData: C.imageData}));
                r(N), N.length !== 0 && w(Math.floor(Math.random() * N.length))
            }
...
function bd({setContact: e, globalContacts: t}) {
    ...
    O.useEffect(() => {
        F.get("/tours/adminpanel/messenger", {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(i => {
    ...
    const o = i => {
        F.delete(`/tours/adminpanel/messenger?email=${i}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(l => {
...
function Qg({contact: e, setGlobalContacts: t}) {
    const [n, r] = O.useState([]), [o, i] = O.useState(""), [l, s] = O.useState(null), [u, a] = O.useState(!1), [d, c] = O.useState(null),
        m = O.useRef(null);
    O.useEffect(() => {
        const x = new WebSocket("/messenger");
        return c(x), x.onmessage = function (h) {
            const p = JSON.parse(h.data);
            p.sender === "server" ? s(p.message) : (p.received = !0, r(v => [...v, p]), F.get("/tours/adminpanel/messenger", {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(v => {
        ...
        if (l !== null) {
            ...
            F.post("/tours/messenger/login", x, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}})
        }
    }, [l]), O.useEffect(() => {
        e !== null && (a(!0), F.get(`/tours/adminpanel/messenger/messages?email=${e.email}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(x => {
            const h = x.data;
            r([]);
...
const Gg = () => {
    ...
    O.useEffect(() => {
        const g = () => {
            F.get("/statistics", {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(x => {
                const h = x.data;
                t(h.time), r(h.clicks), i(h.users), s(h.messages), a(h.feedbacks)
            }).catch(x => console.error("Error fetching statistics:", x)), F.get("/statistics/countries", {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(x => {
                const h = x.data || [];
                if (Array.isArray(h)) {
    ...
    const m = g => {
        F.delete(`/statistics?statistic=${g}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(y => {
            const x = y.data;
            t(x.time), r(x.clicks), i(x.users), s(x.messages), a(x.feedbacks)
        }).catch(y => console.error("Error clearing statistic:", y))
    }, w = () => {
        F.delete("/statistics/countries", {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}), c([])
    };
    return f.jsxs("div", {
...
function Yg() {
    ...
    function n(r) {
        if (r.key === "Enter" || r === "Enter") {
            ...
            F.post("/tours/adminpanel/login", l).then(s => {
...
function bg({sid: e, onLogin: t, setGlobalEmail: n}) {
    ...
    O.useEffect(() => {
        e !== "" && r !== "" && i !== "" && s !== "" && F.get(`/tours/users?email=${i}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(m => {
    ...
    function c(m) {
        if (m.key === "Enter" || m === "Enter") {
            ...
            F.post("/tours/messenger/login", w, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(g => {
...
function ey() {
    ...
    O.useEffect(() => {
        ...
        e || F.get(`/tours/adminpanel/messenger/messages?email=${o}`, {headers: {Authorization: `${I.get("token") ? I.get("token") : null}`}}).then(x => {
            const h = x.data;
            s([]);
